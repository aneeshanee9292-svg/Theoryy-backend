...
public class EmailServiceImpl implements EmailService {
    ...
    public void sendAdminOrderNotification(Order order) {
        try {
            String adminEmail = "udaykumararipaka@gmail.com"; // Admin email

            MimeMessage mimeMessage = mailSender.createMimeMessage();
            MimeMessageHelper helper = new MimeMessageHelper(mimeMessage, true, "UTF-8");

            helper.setTo(adminEmail);
            helper.setSubject("🛒 New Order #" + order.getId() + " — ₹" + order.getFinalAmount());
            ...
            html.append("<style>");
            html.append("body{font-family:Arial,sans-serif;background:#f5f7fa;margin:0;padding:20px;}");
            html.append(".container{max-width:600px;margin:0 auto;background:#fff;border-radius:10px;overflow:hidden;box-shadow:0 4px 10px rgba(0,0,0,0.1);}");
            html.append(".header{background:linear-gradient(135deg,#1a1a2e,#16213e);color:#fff;text-align:center;padding:25px;}");
            html.append(".header h2{margin:0;font-size:22px;}");
            html.append(".card{background:#fff;border:1px solid #eee;border-radius:8px;padding:15px;margin:15px;}");
            html.append(".card h3{margin:0 0 10px;font-size:15px;color:#1a1a2e;}");
            html.append(".amount{background:linear-gradient(135deg,#e8f5e9,#c8e6c9);text-align:center;padding:18px;font-size:22px;font-weight:bold;color:#2e7d32;border-radius:10px;margin:15px;}");
            html.append("table{width:100%;border-collapse:collapse;}th,td{padding:10px;border-bottom:1px solid #eee;font-size:13px;}th{background:#f9f9f9;text-align:left;color:#1a1a2e;}");
            html.append("</style></head><body>");

            html.append("<div class='container'>");
            html.append("<div class='header'><h2>🛒 New Order Received!</h2><p>Order #").append(order.getId()).append("</p></div>");

            // Customer Info
            html.append("<div class='card'><h3>Customer Details</h3>");
            html.append("<p><strong>Name:</strong> ").append(order.getFullName() != null ? order.getFullName() : "N/A").append("</p>");
            html.append("<p><strong>Phone:</strong> ").append(order.getMobileNumber() != null ? order.getMobileNumber() : "N/A").append("</p>");
            html.append("<p><strong>Email:</strong> ").append(order.getEmail() != null ? order.getEmail() : "N/A").append("</p>");
            html.append("<p><strong>Address:</strong> ").append(order.getAddress() != null ? order.getAddress() : "").append(", ")
                .append(order.getCity() != null ? order.getCity() : "").append(", ")
                .append(order.getState() != null ? order.getState() : "").append(" - ")
                .append(order.getPincode() != null ? order.getPincode() : "").append("</p>");
            html.append("</div>");

            // Amount
            html.append("<div class='amount'>Total: ₹").append(order.getFinalAmount()).append("</div>");

            // Items Table
            html.append("<div class='card'><h3>Order Items</h3><table>");
            html.append("<tr><th>Product</th><th>Qty</th><th>Price</th></tr>");
            for (OrderItem item : items) {
                String name = (item.getProduct() != null) ? item.getProduct().getName() : "Product";
                html.append("<tr><td>").append(name).append("</td><td>").append(item.getQuantity())
                    .append("</td><td>₹").append(item.getFinalPrice()).append("</td></tr>");
            }
